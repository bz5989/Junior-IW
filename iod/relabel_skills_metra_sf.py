    ...
    def _update_rewards(self, tensors, v):
        ...
        if self.inner:
            ...
            if self.log_sum_exp:
                ...
                # Apply log-sum-exp for option selection as before
                log_sum_exp = torch.logsumexp(pairwise_scores, dim=-1)

            # Calculate rewards for discrete and continuous options as before
            if self.discrete:
            ...
            # For dual objectives, update the trajectory encodings
            v.update({
                'cur_z': cur_z,
                'next_z': next_z,
            })
            ...
            # For non-inner and non-metra_mlp_rep, no relabeling applied here
        ...
        # Update tensors for tracking reward mean and standard deviation
        tensors.update({
            'PureRewardMean': rewards.mean(),
            'PureRewardStd': rewards.std(),
        })

        # Save the computed rewards in v for later use
        v['rewards'] = rewards
        if self.log_sum_exp:
